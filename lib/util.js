...
function addHTMLReport(jsonData, baseName, options) {
    var basePath = path.dirname(baseName),
        ...
        jsInFile = path.join(__dirname, 'app.js'),
        // Output files
    ...
    // Create a copy of the angular app
    fs.createReadStream(jsInFile).pipe(fs.createWriteStream(jsFile));
...
function addMetaData(metaData, baseName, descriptions, options) {
    ...
    try {
        ...
        stream.append(JSON.stringify(json))
        stream.end();
